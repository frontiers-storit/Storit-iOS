//
//  FullBannerAdViewController.swift
//  Storit
//
...
class FullBannerAdViewController: UIViewController {
    
    let adInfo: AMFullBannerAdInfo
    lazy var interstitial = AMBannerAdViewController(adInfo: self.adInfo, rootViewController: self)
    let buttonStack = UIStackView()
    
    init(adInfo: AMFullBannerAdInfo) {
        self.adInfo = adInfo
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "AdMixer - 전면배너"
        
        setLayout()
    }
}

extension FullBannerAdViewController {
    func setLayout() {
    ...
    @objc func tapShowButton() {
        interstitial.showAd(animated: true)
    }
}

@available(iOS 13.0, *)
struct FullBannerAdViewControllerWrapper: UIViewControllerRepresentable {
    let adInfo: AMFullBannerAdInfo
    
    func makeUIViewController(context: Context) -> UIViewController {
        return UINavigationController(rootViewController: FullBannerAdViewController(adInfo: adInfo))
    }
